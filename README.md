...
```
输入出生日期 + 时辰 + 性别 → 推算四柱 → 五行分析 → 日主性格解读
```

### 干支计算的精度说明

- 日柱以 1900-01-01（甲戌日）为锚点**精确推算**，与万年历一致；
- 年柱以**立春为界**（立春前按上一年计）；
- 月柱按「五虎遁」配合节气定支，时柱按「五鼠遁」；
- 节气采用近似固定日期（如立春按 2/4），与实际节气可能相差 1 天；农历日期为近似值，仅供参考。
...
核心算法（64 卦二进制与卦变关系、干支计算、黄历一致性、纳甲世应六亲、体用生克）均有单元测试：
