...
describe('干支计算', () => {
  ...
  it('月柱五虎遁与节气定支正确', () => {
    ...
    expect(pillarLabel(getMonthPillar(0, 2024, 2, 10))).toBe('丙寅');
    // 2026-07-31（小暑后）丙午年 → 乙未月
    expect(pillarLabel(getMonthPillar(2, 2026, 7, 31))).toBe('乙未');
    // 1900-01-01（小寒前）己亥年 → 丙子月
    expect(pillarLabel(getMonthPillar(5, 1900, 1, 1))).toBe('丙子');
    // 2024-01-15（小寒后、立春前）甲年 → 丁丑月
    expect(pillarLabel(getMonthPillar(0, 2024, 1, 15))).toBe('丁丑');
  });

  it('月支边界：小寒前为子月，小寒当日为丑月', () => {
    expect(getMonthBranchIndex(2024, 1, 5)).toBe(0); // 子
    expect(getMonthBranchIndex(2024, 1, 6)).toBe(1); // 丑
    expect(getMonthBranchIndex(2024, 2, 3)).toBe(1); // 丑
    expect(getMonthBranchIndex(2024, 2, 4)).toBe(2); // 寅
  });

  it('节气名正确（24 节气，含中气）', () => {
    expect(getSolarTermName(2026, 7, 31)).toBe('大暑');
    expect(getSolarTermName(2024, 2, 10)).toBe('立春');
    expect(getSolarTermName(2026, 1, 1)).toBe('冬至');
    expect(getSolarTermName(2026, 8, 8)).toBe('立秋');
  });
